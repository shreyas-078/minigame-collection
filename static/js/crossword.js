...
const wordsDict = {
  ...
  endangered:
    "Species at risk of extinction due to habitat loss or other factors",
};
...
function placeOtherWords(matchOrientation = false) {
  ...
  for (let i = 1; i < usableWords.length; i++) {
    ...
    if (intersections.length > 0) {
      ...
      const orientationToPlace =
        orientations.across < orientations.down ? "across" : "down";
      ...
      for (let j = 0; j < intersections.length; j++) {
        ...
        if (
          matchOrientation &&
          intersection.orientationToPlace !== orientationToPlace
        ) {
          continue;
        }
...
function checkIfPlaceable(word, intersection) {
  ...
  if (orientationToPlace === "down") {
    ...
    for (let j = 0; j < wordLength; j++) {
      ...
      if (
        gridValues[startY + j][startX] !== "" &&
        gridValues[startY + j][startX] !== word[j]
      ) {
        return false;
      }
      if (
        (startX - 1 >= 0 && gridValues[startY + j][startX - 1] !== "") ||
    ...
    if (
      (startY > 0 && gridValues[startY - 1][startX] !== "") ||
      (startY + wordLength < gridSize &&
        gridValues[startY + wordLength][startX] !== "")
    ) {
      return false;
    }
    return true;
    ...
    for (let j = 0; j < wordLength; j++) {
      ...
      if (
        gridValues[startY][startX + j] !== "" &&
        gridValues[startY][startX + j] !== word[j]
      ) {
        return false;
      }
      if (
        (startY - 1 >= 0 && gridValues[startY - 1][startX + j] !== "") ||
    ...
    // Check for adjacent words
    if (
      (startX > 0 && grid.rows[startY].cells[startX - 1].innerText !== "") ||
      (startX + wordLength < gridSize &&
        grid.rows[startY].cells[startX + wordLength].innerText !== "")
    ) {
      return false;
    }
    if (
      (startX > 0 && gridValues[startY][startX - 1] !== "") ||
      (startX + wordLength < gridSize &&
        gridValues[startY][startX + wordLength] !== "")
    ) {
      return false;
    }
    return true;
...
function placeWord(word, intersection) {
  ...
  placedWords.push({
    ...
    start: { y: startY, x: startX },
      ...
      orientationToPlace === "down"
        ? { y: startY + wordLength - 1, x: startX }
        : { y: startY, x: startX + wordLength - 1 },
...
function findIntersections(word) {
  ...
  for (let i = 0; i < placedWords.length; i++) {
    ...
    const orientationToPlace =
      placedWord.orientation === "across" ? "down" : "across";
    ...
    for (let j = 0; j < placedWordLength; j++) {
      if (word.includes(placedWord.word[j])) {
        intersections.push({
          ...
          y:
            orientationToPlace === "across"
              ? placedWord.start.y + j
              : placedWord.start.y,
          x:
            orientationToPlace === "down"
              ? placedWord.start.x + j
              : placedWord.start.x,
          ...
          if (placedWordsForCell.length === 1) {
            ...
            if (orientation === "across") {
              if (e.target.cellIndex + 1 < gridSize) {
                grid.rows[e.target.parentNode.rowIndex].cells[
                  e.target.cellIndex + 1
                ].focus();
              }
            } else {
              if (e.target.parentNode.rowIndex + 1 < gridSize) {
                grid.rows[e.target.parentNode.rowIndex + 1].cells[
                  e.target.cellIndex
                ].focus();
              }
            }
            ...
            if (placedWordsForCell.length === 2) {
              ...
              if (
                ...
                if (
                  grid.rows[currentCellY].cells[currentCellX - 1]
              ...
              if (
                ...
                if (
                  grid.rows[currentCellY - 1].cells[currentCellX]
                    .contentEditable === "true"
                ) {
                  upEmpty = 0;
                } else {
        ...
        if (e.key === "Backspace") {
          ...
          if (currentCell.innerText === "") {
            ...
            if (
              ...
              if (
                currentCell.parentNode.rowIndex - 1 >= 0 &&
...
function solveGrid() {
  ...
  const confirmSolve = confirm(
    "Are you sure you want to solve the grid?\nYou won't earn any xp if you do!"
  );
  if (!confirmSolve) {
    return;
  }
...
function calculateScore(userSolved = true) {
    ...
    for (let i = 0; i < placedWords.length; i++) {
      ...
      if (orientation === "across") {
        for (let j = 0; j < word.length; j++) {
          ...
          if (
            word[j] !==
            grid.rows[start.y].cells[start.x + j].innerText.slice(0, 1)
          ) {
            correctWord = false;
            break;
          }
        ...
        // Check if the user's answer is correct for down words
        for (let j = 0; j < word.length; j++) {
          // Slice the first character of the cell's inner text to remove the hint number
          if (
            word[j] !==
            grid.rows[start.y + j].cells[start.x].innerText.slice(0, 1)
          ) {
            correctWord = false;
            break;
          }
  ...
  if (correctWords < 6) {
    scoreDiv.innerText += ". Refresh to try again and get them all!";
    const refreshButton = document.createElement("button");
    refreshButton.style =
      "height: 5rem; width: 12rem; font-size: 1.2rem; padding: 0.5rem; position: fixed; top = 5px; left = 5px;";
  ...
  if (correctWords === placedWords.length) {
    ...
    setTimeout(() => {
      fetch("/update-stage", {
        ...
        body: JSON.stringify({ curStage: "2" }),
      })
        .then((res) => res.json())
